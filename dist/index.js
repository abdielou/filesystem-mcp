...
server.registerTool("list_directory_with_sizes", {
    ...
    // Format the output
    const formattedEntries = paged.map(entry => `${entry.isDirectory ? "[DIR]" : "[FILE]"} ${entry.name.padEnd(30)} ${entry.isDirectory ? "" : formatSize(entry.size).padStart(10)}`);
    // Add summary
...
server.registerTool("directory_tree", {
    ...
    let entryCount = 0;
    let truncated = false;
    async function buildTree(currentPath, excludePatterns = [], depth = 0) {
